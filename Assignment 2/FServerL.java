/* Sends requested file contents to the client upon receiving acknowledgement with Lost Frame and Lost Acknowledgement */

import java.io.*;
import java.net.*;
import java.util.*;

public class FServerL {
        ...
        try {
          ...
          if (consignment == frame) {
            ...
            if (result < 512) {
              ...
              mymsg = concatenateByteArrays(RDT, id, sd, END, CRLF);
              result = -1;
            } else mymsg = concatenateByteArrays(RDT, id, td, CRLF);
            sp = new DatagramPacket(mymsg, mymsg.length, ip, port);
            ss.send(sp);
